...
def get_morph_dict():
    ...
    pronoun_classes = ['noun_bound_pronoun', 'verb_bound_pronoun', 'preposition_bound_pronoun',
                       'object_marker_bound_pronoun', 'relative_pronoun', 'independent_pronoun',
                       'interrogative_pronoun']
    general = ['verbs', 'nouns', 'words', 'particles', 'pronouns', 'adjectives']
    specific_word_counts = ['ky', 'kya', 'hnh', 'whnh', 'aCr', 'oM']
    morph_name_dict = {
        'noun_classes': {x: xx[1] for x, xx in morph_parser.raw_morph_yaml['values']['cl']['subs'].items()},
...
def gen_sample_features(entries, morph_name_dict, feature_list):
    count = {key: {k: 0 for k in morph_name_dict[key].values()} for key in morph_name_dict}
        ...
        if 'sp' in parsed_morph:
            ...
            if parsed_morph['sp'] == 'subs':
                ...
                if 'st' in parsed_morph and entry['parsed_morph']['st'] in morph_name_dict['noun_states']:
                    count['noun_states'][morph_name_dict['noun_states'][entry['parsed_morph']['st']]] += 1
                if entry['parsed_morph']['cl'] in morph_name_dict['noun_classes'].keys():
                    count['noun_classes'][morph_name_dict['noun_classes'][entry['parsed_morph']['cl']]] += 1

            if parsed_morph['sp'] == 'verb':
                count['general']['verbs'] += 1
                if entry['parsed_morph']['vs'] in morph_name_dict['verbal_stems'].keys():
                    count['verbal_stems'][morph_name_dict['verbal_stems'][entry['parsed_morph']['vs']]] += 1
                if 'vt' in parsed_morph and entry['parsed_morph']['vt'] in morph_name_dict['verbal_tense']:
                    count['verbal_tense'][morph_name_dict['verbal_tense'][entry['parsed_morph']['vt']]] += 1
                ...
                if 'md' in parsed_morph and entry['parsed_morph']['md'] in morph_name_dict['verbal_mood']:
                    count['verbal_mood'][morph_name_dict['verbal_mood'][entry['parsed_morph']['md']]] += 1
            if parsed_morph['sp'] == 'ptcl':
                count['general']['particles'] += 1
                count['particle_classes'][
                    morph_name_dict['particle_classes'][entry['parsed_morph']['cl']]] += 1

            if parsed_morph['sp'] == 'adjv':
                count['general']['adjectives'] += 1
                if 'st' in parsed_morph:
                    count['adjective_states'][
                        morph_name_dict['adjective_states'][entry['parsed_morph']['st']]] += 1
...
def get_starr_features(samples):
    ...
    features = [gen_sample_features(sample, morph_dict, feature_list) for sample in samples]
