...
samples, sample_names = zip(*[x for x in parser_data.gen_samples(filtered_data['11Q5'], n_words_per_feature)])
features = [gen_sample_features(sample, morph_name_dict, feature_list) for sample in samples]
...
for preset in presets:
    model = sk.AgglomerativeClustering(distance_threshold=0, n_clusters=None, linkage=preset[0])
    clusters = model.fit_predict(features)
    file_name = 'Starr Features, {}, {}'.format(n_words_per_feature, preset[0])
    title = 'Clustering with Morphological Features, {} words per sample, linakage: {}'.format(n_words_per_feature, preset[0])
    plot_dendrogram(model, sample_names, adjacency_matrix, title, path.join(output_file_path, file_name))
    ...
    print(f"Random {preset}: mean: {np.mean(random_scores)}, std: {np.std(random_scores)}")
