...
BIBLE_DATA_CSV = f"{BIBLE_DATA_PATH}/df_CHUNK_SIZE=100_MAX_OVERLAP=10_2024_15_01.csv"
BIBLE_PROCESSED_VECTORIZERS_PATH = f"{BIBLE_DATA_PATH}/processed_vectorizers.pkl"
...
def get_paths_by_domain(domain):
    ...
    if domain == "bible":
        return {
            "data_path": BIBLE_DATA_PATH,
            "data_csv_path": BIBLE_DATA_CSV,
            "processed_vectorizers_path": BIBLE_PROCESSED_VECTORIZERS_PATH,
        }
    else:
        raise ValueError(f"Invalid domain: {domain} (only dss and bible implemented)")
