...
def logistic_regressions(samples, sample_names, labels, sec_test_books, non_sec_test_books, class_weight, sample_weight):
    name = [x.split(":")[0] for x in sample_names]
    # test_books = sec_test_books + non_sec_test_books
    train = [i for i in range(len(samples)) if name[i] not in (sec_test_books + non_sec_test_books)]
    test = [i for i in range(len(samples)) if name[i] in (sec_test_books + non_sec_test_books)]

    np.random.shuffle(train)
    np.random.shuffle(test)
    classifier = linear_model.LogisticRegression(max_iter=300, class_weight=class_weight)
    classifier = classifier.fit(samples[train], labels[train], sample_weight=sample_weight)
    roc_auc = roc_auc_score(labels[test], classifier.predict_proba(samples[test])[:, 1], multi_class='ovr',
                            labels=['sectarian_texts', 'non_sectarian_texts'])
    prediction = classifier.predict(samples[test])
    accuracy = np.sum([prediction[i] == labels[test[i]] for i in range(len(test))]) / len(test)
    true_positive = np.sum([1 for i in range(len(test)) if prediction[i] == 'sectarian_texts' and labels[test][i] == 'sectarian_texts'])
    false_positive = np.sum([1 for i in range(len(test)) if prediction[i] == 'sectarian_texts' and labels[test][i] == 'non_sectarian_texts'])
    true_negative = np.sum([1 for i in range(len(test)) if prediction[i] == 'non_sectarian_texts' and labels[test][i] == 'non_sectarian_texts'])
    false_negative = np.sum([1 for i in range(len(test)) if prediction[i] == 'non_sectarian_texts' and labels[test][i] == 'sectarian_texts'])
...
def run_experiment(number_expr):
    ...
    non_sectarian_books = [k for k, val in non_sectarian_dict.items() if val > 4 and val < 12]
    res_benchmark_acc = []
    res_benchmark_roc = []
    res_bert_acc = []
    res_bert_roc = []
    ...
    for j in tqdm(range(number_expr)):
        ...
        trigram_samples, trigram_sample_names, trigram_labels = get_trigram_feature_vectors(test_books)
        name = [x.split(":")[0] for x in trigram_sample_names]
        book_counter = Counter(name)
        train_category = ['sectarian_text' if name[i] in sectarian_dict else 'non_sectarian_text'
        ...
        class_weight = {'sectarian_texts': 1, 'non_sectarian_texts': type_counter['sectarian_text'] / type_counter['non_sectarian_text']}
        # print(f"sectarian_book:{str(non_sectarian_books[:4])}, non_sectarian_book:{str(non_sectarian_books[:2])}")
        # print("Benchmark")
        accuracy, roc_auc = logistic_regressions(samples=trigram_samples, sample_names=trigram_sample_names,
                                                 labels=trigram_labels, sec_test_books=sectarian_books[:4],
                                                 non_sec_test_books=non_sectarian_books[:2], class_weight=class_weight,
        ...
        book_counter = Counter(name)
        samples_weight = [1 / book_counter[n] for n in name if n not in test_books]
        accuracy, roc_auc = logistic_regressions(samples=bert_samples, sample_names=bert_sample_names,
                                                 labels=bert_labels, sec_test_books=sectarian_books[:4],
                                                 non_sec_test_books=non_sectarian_books[:2], class_weight=class_weight,
                                                 sample_weight=samples_weight)

        res_bert_acc.append(accuracy)
        res_bert_roc.append(roc_auc)
    print(f"benchmark acc - mean: {np.mean(res_benchmark_acc)}, std: {np.std(res_benchmark_acc)}, min: {np.min(res_benchmark_acc)}")
    print(f"benchmark roc - mean: {np.mean(res_benchmark_roc)}, std: {np.std(res_benchmark_roc)}, min: {np.min(res_benchmark_roc)}")
    print(f"bert acc - mean: {np.mean(res_bert_acc)}, std: {np.std(res_bert_acc)}, min: {np.min(res_bert_acc)}")
    print(f"bert roc - mean: {np.mean(res_bert_roc)}, std: {np.std(res_bert_roc)}, min: {np.min(res_bert_roc)}")
