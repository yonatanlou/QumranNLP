    ...
    for section in section_type:
        ...
        for book_name, book_data in data.items():
            ...
            samples, sample_names = parser_data.get_samples(
                book_data, word_per_samples=150
            )
...
def get_trigram_():
    ...
    (
        ...
        trigram_samples,
        labels,
    ) = get_trigram_feature_vectors()
    sectarian_samples = [
        i for i, labels in enumerate(labels) if labels == "sectarian_texts"
    ]
    non_sectarian_samples = [
        i for i, labels in enumerate(labels) if labels != "sectarian_texts"
    ]
    ...
    train = (
        sectarian_samples[:train_idx_sectarian]
        + non_sectarian_samples[:train_idx_non_sectarian]
    )
    test = (
        sectarian_samples[train_idx_sectarian:]
        + non_sectarian_samples[train_idx_non_sectarian:]
    ...
    clf = svm.SVC()
    clf.fit(
        trigram_samples[train],
        labels[train],
        sample_weight=[1 if x == "sectarian_texts" else 8 for x in labels[train]],
    )
...
def get_benchmark_logistic():
    ...
    classifier = linear_model.LogisticRegression(
        class_weight={"sectarian_texts": 1, "non_sectarian_texts": 7}
    )
