...
def save_words(name, words):
    with open(f"{RESULT_PATH}/{name}.txt", "w") as f:
        latin_words = list(
            set(["".join(filter(str.isalnum, x["transcript"])) for x in words])
        )
...
def parser_book():
    ...
    total_words = len(
        [
            ...
            for x in data
            if x["sub_word_num"] == "1"
            and "".join(filter(str.isalnum, x["transcript"])) != ""
        ]
    )  # N
    ...
    num_of_cp_words = sum([len(x["rectums"]) for x in filtered_map_list]) + len(
        filtered_map_list
    )  # D
    filtered_properties = [
        properties_list[i]
        for i in map_list
        if map_list[i]["book"] not in BIBLE_LIST and i != 4297
    ]
    aggr_properties = {
        "Nog": len(
            [x["N"] for x in filtered_properties if x["NoGRec"] != 0]
            + [x["N"] for x in filtered_properties if x["NoGReg"] != 0]
        ),
        "Quant": len(
            [x["N"] for x in filtered_properties if x["Quantmid"] != 0]
        ...
        "partpas": len(
            [x["N"] for x in filtered_properties if x["RecpartPas"] != 0]
            + [x["N"] for x in filtered_properties if x["RegpartPas"] != 0]
        ),
        "partpf": len(
            [x["N"] for x in filtered_properties if x["Recpartpf"] != 0]
            + [x["N"] for x in filtered_properties if x["Regpartpf"] != 0]
        ),
        "partpm": len(
            [x["N"] for x in filtered_properties if x["Recpartpm"] != 0]
            + [x["N"] for x in filtered_properties if x["Regpm"] != 0]
        ),
        "partsf": len(
            [x["N"] for x in filtered_properties if x["Recpartsf"] != 0]
            + [x["N"] for x in filtered_properties if x["Regsf"] != 0]
        ),
        "partsm": len(
            [x["N"] for x in filtered_properties if x["Recpartsm"] != 0]
    ...
    with open(f"{RESULT_PATH}/p_value_results.csv", "w", newline="") as f:
        ...
        csvwriter.writerow(
            ["Category", "Total_number", "Number_in_PC", "p_value", "expected"]
        )
        # god name:
        total_god_name = get_god_name(data)  # n
        p_value = get_p_value(
            k=aggr_properties["Nog"] - 1,
            N=total_words,
            D=num_of_cp_words,
            n=len(total_god_name),
        )
        expected = num_of_cp_words * len(total_god_name) / total_words
        csvwriter.writerow(
            ["God name", len(total_god_name), aggr_properties["Nog"], p_value, expected]
        )
        ...
        p_value = get_p_value(
            k=aggr_properties["Quant"] - 1,
            N=total_words,
            D=num_of_cp_words,
            n=len(quant_num),
        )
        csvwriter.writerow(
            ["Quant", len(quant_num), aggr_properties["Quant"], p_value, expected]
        )
        ...
        p_value = get_p_value(
            k=aggr_properties["conj"] - 1,
            N=total_words,
            D=num_of_cp_words,
            n=len(conj_num),
        )
        csvwriter.writerow(
            ["Conj", len(conj_num), aggr_properties["conj"], p_value, expected]
        )
        ...
        p_value = get_p_value(
            k=aggr_properties["prep"] - 1,
            N=total_words,
            D=num_of_cp_words,
            n=len(prep_num),
        )
        csvwriter.writerow(
            ["Prep", len(prep_num), aggr_properties["prep"], p_value, expected]
        )
        ...
        p_value = get_p_value(
            k=aggr_properties["partpas"] - 1,
            N=total_words,
            D=num_of_cp_words,
            n=len(part_num),
        )
        csvwriter.writerow(
            ["passive", len(part_num), aggr_properties["partpas"], p_value, expected]
        )
        ...
        expected = num_of_cp_words * len(part_num) / total_words
        p_value = get_p_value(
            k=aggr_properties["partpf"] - 1,
            N=total_words,
            D=num_of_cp_words,
            n=len(part_num),
        )
        csvwriter.writerow(
            ["pf", len(part_num), aggr_properties["partpf"], p_value, expected]
        )
        ...
        expected = num_of_cp_words * len(part_num) / total_words
        p_value = get_p_value(
            k=aggr_properties["partpm"] - 1,
            N=total_words,
            D=num_of_cp_words,
            n=len(part_num),
        )
        csvwriter.writerow(
            ["pm", len(part_num), aggr_properties["partpm"], p_value, expected]
        )
        ...
        expected = num_of_cp_words * len(part_num) / total_words
        p_value = get_p_value(
            k=aggr_properties["partsf"] - 1,
            N=total_words,
            D=num_of_cp_words,
            n=len(part_num),
        )
        csvwriter.writerow(
            ["sf", len(part_num), aggr_properties["partsf"], p_value, expected]
        )
        ...
        expected = num_of_cp_words * len(part_num) / total_words
        p_value = get_p_value(
            k=aggr_properties["partsm"] - 1,
            N=total_words,
            D=num_of_cp_words,
            n=len(part_num),
        )
        csvwriter.writerow(
            ["sm", len(part_num), aggr_properties["partsm"], p_value, expected]
        )
...
def quant_counter(data):
    ...
    kl = [x for x in data if x["transcript"] == "kl"] + [
        x for x in data if x["transcript"] == "kwl"
    ]
...
def get_god_name(data):
    ...
    al = [
        x
        for x in data
        if "".join(filter(str.isalnum, x["transcript"]))[:2] == "al"
    ...
    al = [
        x
        for x in al
        if len("".join(filter(str.isalnum, x["transcript"]))) == 2
        or "".join(filter(str.isalnum, x["transcript"]))[2] in ["y", "h", "w"]
    ]
    yh = [
        x
        for x in data
        if "".join(filter(str.isalnum, x["transcript"])) == "yh"
        and x["parsed_morph"].get("sp") == "subs"
    ]
    olywn = [
        x
        for x in data
        if "".join(filter(str.isalnum, x["transcript"])) == "olywn"
        and x["parsed_morph"].get("sp") == "subs"
    ]
    kadosh = [
        x
        for x in data
        if "".join(filter(str.isalnum, x["transcript"])) == "qdwC"
        and x["parsed_morph"].get("sp") == "subs"
    ]
...
def get_passive(data):
    ...
    return [
        x
        for x in data
        if x["parsed_morph"].get("vt", "") == "ptcp"
        or (
            x["parsed_morph"].get("vt", "") == "ptca"
            and x["parsed_morph"].get("vs") in ["passive", "pual", "nifal"]
        )
    ]
