...
def import_data(date):
    ...
    book_name = {
        ...
        "1QSa": "1QSa",
        "1Q26": "Musar_Lamevin",
        "4Q418b": "Musar_Lamevin",
        "4Q423": "Musar_Lamevin",
        "4Q418": "Musar_Lamevin",
        "4Q416": "Musar_Lamevin",
        "4Q417": "Musar_Lamevin",
        "4Q418c": "Musar_Lamevin",
        "1Q35": "hodayot",
    ...
    with open(f"data/CP/{date}/properties.csv", newline="") as f:
        ...
        for index, row in df.iterrows():
            row_dict = {
                c.replace(" ", ""): get_val(str(row[c]), row[0], c) for c in df.columns
            }
...
def get_statistic_for_group(map_list, properties_list, idx, normalized_factor=1):
    ...
    count = len(idx)
    print(count)
    properties_statistic = {
        "total_words": normalized_factor,
        "construct_count": count,
        "percent": count / normalized_factor,
    }
...
def create_graphs(first_statistic, second_statistic, first_name, second_name, date):
    ...
    axs[0].bar(
        [i for i in range(len(first_statistic["MultRec"]))], first_statistic["MultRec"]
    )
    axs[1].bar(
        [i for i in range(len(second_statistic["MultRec"]))],
        second_statistic["MultRec"],
    )
    ...
    axs[0].bar(
        [i for i in range(len(first_statistic["Multregs"]))],
        first_statistic["Multregs"],
    )
    axs[1].bar(
        [i for i in range(len(second_statistic["Multregs"]))],
        second_statistic["Multregs"],
    )
    ...
    plt.bar(
        [w[0] for w in word_counter[:30]],
        [w[1] / first_statistic["construct_count"] for w in word_counter[:30]],
    )
    ...
    plt.bar(
        [w[0] for w in word_counter[:30]],
        [w[1] / second_statistic["construct_count"] for w in word_counter[:30]],
    )
    plt.xticks(rotation=50)
...
def get_statistics(date):
    ...
    normalize_prose = 1
    normalize_poem = 1
    prose_statistic = get_statistic_for_group(
        map_list, properties_list, prose, normalize_prose
    )
    poem_statistic = get_statistic_for_group(
        map_list, properties_list, poem, normalize_poem
    )
    create_graphs(prose_statistic, poem_statistic, "Prose", "Poem", date)

    ## bible v.s scrolls
    normalize_scrolls = sum(
        [v for k, v in number_of_words.items() if k in scrolls_books]
    )
    ...
    bible = [
        i
        for i, val in map_list.items()
        if val["ref"].split(" ")[0].split(":")[0] in bible_refs
    ]
    scroll = [i for i in map_list if i not in bible]
    bible_statistic = get_statistic_for_group(
        map_list, properties_list, bible, normalize_bible
    )
    scroll_statistic = get_statistic_for_group(
        map_list, properties_list, scroll, normalize_scrolls
    )
    ...
    with open(f"{BASE_DIR}/results/CP/{date}/prose_poem.csv", "w") as f:
        ...
        for p in properties:
            writer.writerow(
                {"property": p, "prose": prose_statistic[p], "poem": poem_statistic[p]}
            )
    ...
    with open(f"{BASE_DIR}/results/CP/{date}/bible_scrolls.csv", "w") as f:
        ...
        for p in properties:
            writer.writerow(
                {
                    "property": p,
                    "bible": bible_statistic[p],
                    "scrolls": scroll_statistic[p],
                }
            )
...
def tmp(map_list, properties_list, is_bible, date):
    ...
    book = [
        i
        for i, val in map_list.items()
        if (val["ref"].split(" ")[0].split(":")[0] in bible_refs) == is_bible
    ]
    ...
    prose_regen_word_counter = sorted(
        prose_regen_word_counter, reverse=True, key=lambda x: x[1]
    )
    prose_rectum = Counter(
        [y for i, x in map_list.items() if i in prose for y in x["rectums"]]
    )
    prose_rectum_word_counter = list(prose_rectum.items())
    prose_rectum_word_counter = sorted(
        prose_rectum_word_counter, reverse=True, key=lambda x: x[1]
    )
    ...
    poem_regen_word_counter = sorted(
        poem_regen_word_counter, reverse=True, key=lambda x: x[1]
    )
    poem_rectum = Counter(
        [y for i, x in map_list.items() if i in poem for y in x["rectums"]]
    )
    poem_rectum_word_counter = list(poem_rectum.items())
    poem_rectum_word_counter = sorted(
        poem_rectum_word_counter, reverse=True, key=lambda x: x[1]
    )
    words_to_counter = (
        prose_regen_word_counter[:10]
        + prose_rectum_word_counter[:10]
        + poem_regen_word_counter[:10]
        + poem_rectum_word_counter[:10]
    )
    ...
    ax.bar(
        words_to_counter,
        poem_regen_most_freq,
        bottom=prose_regen_most_freq,
        label="regen: poem",
    )
    ax.bar(
        words_to_counter,
        prose_rectum_most_freq,
        bottom=np.add(prose_regen_most_freq, poem_regen_most_freq),
        label="rectum: prose",
    )
    ax.bar(
        words_to_counter,
        poem_rectum_most_freq,
        bottom=np.add(
            np.add(prose_regen_most_freq, prose_rectum_most_freq), poem_regen_most_freq
        ),
        label="rectum: poem",
    )
    plt.xticks(rotation=50)
    plt.legend()
    plt.title(
        f"10 most frequency words from each category: {'bible' if is_bible else 'scrolls'}"
    )
    plt.savefig(
        f"{BASE_DIR}/results/CP/{date}/most_frequency_words_{'bible' if is_bible else 'scrolls'}"
    )
        ...
        for v in G.nodes:
            writer.writerow(
                [
                    ...
                    G.out_degree[v],
                    G.in_degree[v],
                    G.out_degree[v] / (G.out_degree[v] + G.in_degree[v]),
                ]
            )
