...
def df_conversion(all_psukim, all_labels, book_to_label):
    ...
    logger.info(
        f"words by label: {label_stats.reset_index().to_dict(orient='records')}"
    )
...
def write_data_into_text_files(
    ...
    if label not in ("sectarian", "book", "bib_nonbib"):
        raise ValueError(f"Please choose either 'sectarian' or 'book'")
    book_to_label = generate_book_to_label(book_dict, book_to_section)
...
def generate_book_to_label(book_dict, book_to_section):
    res = []
    for book, lst_of_chapters in book_dict.items():
        for chapter in lst_of_chapters:
            res.append({"label": book_to_section.get(book), "book": chapter})
