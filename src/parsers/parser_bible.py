...
def get_verbs_categories(filtered_data):
    ...
    first_type_v = [
        x for x in tmp if x["morph"][0] == "V" and len(x["morph"].split("/")[0]) == 6
    ]
    options = collections.defaultdict(list)
    [options[x["morph"][1:3]].append(x["transcript"]) for x in first_type_v]
    second_type_v = [
        x for x in tmp if x["morph"][0] == "V" and len(x["morph"].split("/")[0]) == 3
    ]
    ...
    with open(DATA) as f:
        for r in f.readlines():
            ...
            for i, _ in enumerate(words_idx):
                ...
                word_line_num = (
                    word_num if len(words_idx) == 1 else f"{word_num}.{i + 1}"
                )
                ...
                filtered_data[parsed_word[line_fields_names.book_name]].append(
                    parsed_word
                )
