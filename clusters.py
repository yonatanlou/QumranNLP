...
def get_clusters_scores(
    ...
    file_name = f"{path}/{method_name}_{word_per_samples}_samples_{linkage_criterion}"
    title = f"{method_name}, {word_per_samples} samples, Linkage Criterion: {linkage_criterion}"
    ...
    score = plot_dendrogram(
        model, sample_names, adjacency_matrix, title, file_name
    ...
    print(
        f"{method_name}: clusters scores: {score:.4f},Random: [mean: {np.mean(random_scores):.4f}, std: {np.std(random_scores):.4f}]"
    )
    book_scores.update(
        {
            f"{method_name}_feature_clusters_score": score,
            f"{method_name}_feature_random_score_mean": np.mean(random_scores),
            f"{method_name}_feature_random_score_std": np.std(random_scores),
        }
    )
    return book_scores
